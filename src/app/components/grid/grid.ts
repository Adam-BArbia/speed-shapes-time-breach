...
export class Grid {
  ...
  readonly teamShapes = computed(() => {
    const map = new Map<string, string>();
    for (const team of this.teams()) {
      map.set(team.id, team.shape);
    }
    return map;
  });
  ...
  readonly connectionLines = computed(() => {
    ...
    for (const conn of this.connections()) {
      ...
      lines.push({ x1: pos1.x, y1: pos1.y, x2: pos2.x, y2: pos2.y, teamColor, isNew });
      lines.push({ x1: pos2.x, y1: pos2.y, x2: pos3.x, y2: pos3.y, teamColor, isNew });
    }
  ...
  cellColor(cell: Cell): string {
    if (!cell.teamId) return '#7f8aa3';
    return this.teamColors().get(cell.teamId) ?? '#7f8aa3';
  }
 
  cellShape(cell: Cell): string {
    if (!cell.teamId) return '';
    return this.teamShapes().get(cell.teamId) ?? '';
  }
